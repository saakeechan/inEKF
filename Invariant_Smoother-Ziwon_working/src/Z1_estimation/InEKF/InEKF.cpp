...
InEKF::InEKF(NoiseParams params) : g_((Eigen::VectorXd(3) << 0,0,-9.80665).finished()), noise_params_(params) {}

// Constructor with initial state
InEKF::InEKF(RobotState state) : g_((Eigen::VectorXd(3) << 0,0,-9.80665).finished()), state_(state) {}

// Constructor with initial state and noise params
InEKF::InEKF(RobotState state, NoiseParams params) : g_((Eigen::VectorXd(3) << 0,0,-9.80665).finished()), state_(state), noise_params_(params) {}
...
void InEKF::Propagate(const Eigen::Matrix<double,6,1>& m, double dt, std::vector<double> contact_cov_array) {
    ...
    for(map<int,int>::iterator it=estimated_contact_positions_.begin(); it!=estimated_contact_positions_.end(); ++it) {
        Qk.block<3,3>(3+3*(it->second-3),3+3*(it->second-3)) = contact_cov_array[it->first] * Eigen::MatrixXd::Identity(3,3); // Contact noise terms
    }
