...
#ifndef SRC_Traditional_ESTIMATOR_HPP
#define SRC_Traditional_ESTIMATOR_HPP
...
class Traditional_estimator {

public:

    int NUM_OF_TRASH_DATA = 1;

    int   state_size[(WINDOW_SIZE + 1)];
    int   state_idx[(WINDOW_SIZE + 1)];

    int   para_size[(WINDOW_SIZE + 1)];
    int   para_idx[(WINDOW_SIZE + 1)];

    const static int num_z_imu=6;
    const static int num_z_encoder=12;
    const static int num_z_encoderdot=12;
    const static int index_z_imu = 0;
    const static int index_z_encoder = 6;
    const static int num_z = num_z_imu + num_z_encoder + num_z_encoderdot;



    Eigen::Matrix<double,Eigen::Dynamic,1>                      Estimation_X;
    Eigen::Matrix<double,Eigen::Dynamic,1>                      Estimation_X_init;
    Eigen::Matrix<double,Eigen::Dynamic,1>                      Estimation_dX;
    Eigen::Matrix<double,Eigen::Dynamic,1>                      Estimation_dX_init;
    ...
    void Estimation_Gradient_Hessian_Cost(Eigen::Matrix<double, Eigen::Dynamic,1> &X,
    ...
    void Gradient_Hessian_Update_For_Marginalization(Eigen::Matrix<double, Eigen::Dynamic,1> &X,
                                                     Eigen::Matrix<double, Eigen::Dynamic,1> &X_init,
                                                     Eigen::Matrix<double, Eigen::Dynamic,1> &dX,
                                                     const Eigen::Matrix<double, num_z * (WINDOW_SIZE + 1), 1> &Z,
                                                     const Eigen::Matrix<double, Eigen::Dynamic,Eigen::Dynamic> &marginalized_H,
    ...
    Vector3d                   Position_s       [(WINDOW_SIZE + 1)];
    Matrix3d                   Rotation_s       [(WINDOW_SIZE + 1)];
    Vector3d                   Velocity_s       [(WINDOW_SIZE + 1)];
    Matrix<double, 12,1>       d_s              [(WINDOW_SIZE + 1)];
    Matrix<double, 12,1>       dv_s             [(WINDOW_SIZE + 1)];

    int contact_leg_num_array                   [(WINDOW_SIZE + 1)];


    Vector3d                   Bias_Acc_s       [(WINDOW_SIZE + 1)];
    Vector3d                   Bias_Gyro_s      [(WINDOW_SIZE + 1)];


    // True Values and measurement Buffer

    Eigen::Matrix<bool, 4,1>   HARD_CONTACT_t  [(WINDOW_SIZE + 1)];
    Eigen::Matrix<bool, 4,1>   CONTACT_t       [(WINDOW_SIZE + 1)];
    Eigen::Matrix<bool, 4,1>   SLIP_t          [(WINDOW_SIZE + 1)];

    Eigen::Matrix<double, 3,1>         IMU_Gyro         [(WINDOW_SIZE + 1)];
    Eigen::Matrix<double, 3,1>         IMU_Acc          [(WINDOW_SIZE + 1)];
    Eigen::Matrix<double,12,1>         ENCODER          [(WINDOW_SIZE + 1)];
    Eigen::Matrix<double,12,1>         ENCODERDOT       [(WINDOW_SIZE + 1)];


    std::vector<Eigen::Matrix3d> Rotation_container;
    ...
    int idx_jac_std                 = idx_cost + 4;//103
    int idx_end                     = idx_jac_std + 1;//104
...
#endif //SRC_Traditional_ESTIMATOR_HPP
