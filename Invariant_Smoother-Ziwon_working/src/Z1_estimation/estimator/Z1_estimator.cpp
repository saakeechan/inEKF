...
void Z1_estimator::Call_File(std::string file_name) {
    ...
    myfile.open("../polished_data/" + file_name + "_sensordata.txt");
    ...
    myfile.open("../polished_data/" + file_name + "_groundtruth.txt");
...
void Z1_estimator::Initialize(double _dt, Eigen::Matrix<double, 12, 1> &cov_val_setting,
    ...
    if (variable_contact_cov_mode == true) {
        ...
        if (Retract_All_flag) {
            mode = "(BCH)(SR" + BasicFunctions_Estimator::to_string_n_signficant_figures(slip_threshold, 2) + "_"
    ...
    initial_info = "(" + BasicFunctions_Estimator::to_string_n_figures_under_0(initial_condition(0), 2) + "_" +
                   BasicFunctions_Estimator::to_string_n_figures_under_0(initial_condition(1), 2) + "_"
                   + BasicFunctions_Estimator::to_string_n_figures_under_0(initial_condition(2), 2) + "_" +
                   BasicFunctions_Estimator::to_string_n_figures_under_0(initial_condition(3), 2) + "_"
                   + BasicFunctions_Estimator::to_string_n_figures_under_0(initial_condition(4), 2) + "_" +
                   BasicFunctions_Estimator::to_string_n_figures_under_0(initial_condition(5), 2) + "_"
                   + BasicFunctions_Estimator::to_string_n_figures_under_0(initial_condition(6), 2) + "_" +
                   BasicFunctions_Estimator::to_string_n_figures_under_0(initial_condition(7), 2) + "_"
                   + BasicFunctions_Estimator::to_string_n_figures_under_0(initial_condition(8), 2) + "_" +
                   BasicFunctions_Estimator::to_string_n_figures_under_0(initial_condition(9), 2) + "_"
                   + BasicFunctions_Estimator::to_string_n_figures_under_0(initial_condition(10), 2) + "_" +
                   BasicFunctions_Estimator::to_string_n_figures_under_0(initial_condition(11), 2) + ")";
    ...
    if (textfile_flag) {
        ...
        RS[0].Position << GroundTruth[1 + gt_sd][12] + initial_condition(0), GroundTruth[1 + gt_sd][13] +
                                                                             initial_condition(1),
                GroundTruth[1 + gt_sd][14] + initial_condition(2);
        RS[0].Velocity << GroundTruth[1 + gt_sd][9] + initial_condition(7), GroundTruth[1 + gt_sd][10] +
                                                                            initial_condition(8),
                GroundTruth[1 + gt_sd][11] + initial_condition(9);
...
void Z1_estimator::new_measurement(Eigen::Matrix<double, 30, 1> &Sensor_i, Eigen::Matrix<bool, 4, 1> &Contact_i) {
    ...
    if (frame_count == 0) {//Initializing states which require sensor measurement necessarily
        ...
        for (int k = 0; k < robot.leg_no; k++) {
            RS[0].d.block(3 * k, 0, 3, 1) = RS[0].Position + RS[0].Rotation * robot.Forward_Kinematics_Leg(
                    Estimation_Z.block<3, 1>(6 + 3 * k, 0), k + 1);
        }
        ...
        RS[frame_count].Rotation = RS[frame_count - 1].Rotation * BasicFunctions_Estimator::Expm_Vec(
                (IMU_Gyro_Previous - RS[frame_count - 1].Bias_Gyro) * dt);
        ...
        RS[frame_count].Velocity = RS[frame_count - 1].Velocity
                                   + RS[frame_count -
                                        1].Rotation //* BasicFunctions_Estimator::Left_Jacobian_SO3((IMU_Gyro_Previous - RS[frame_count-1].Bias_Gyro)*dt)
        ...
        RS[frame_count].Position = RS[frame_count - 1].Position + RS[frame_count - 1].Velocity * dt
                                   + 0.5 * RS[frame_count - 1].Rotation *
                                     (IMU_Acc_Previous - RS[frame_count - 1].Bias_Acc) * dt * dt +
                                   0.5 * gravity * dt * dt;
        ...
        for (int k = 0; k < robot.leg_no; k++) {
            ...
            RS[frame_count].d_v.block(3 * k, 0, 3, 1) = RS[frame_count].Velocity
                                                        + RS[frame_count].Rotation *
                                                          robot.Jacobian_Leg(ENCODER.block<3, 1>(3 * k, 0), k + 1) *
                                                          ENCODERDOT.block<3, 1>(3 * k, 0)
                                                        + RS[frame_count].Rotation * BasicFunctions_Estimator::Hat_so3(
                    IMU_Gyro - RS[frame_count].Bias_Gyro) * robot.Forward_Kinematics_Leg(ENCODER.block<3, 1>(3 * k, 0),
                                                                                         k + 1);
...
//            RS[frame_count].Hard_Contact(k)  = RS[frame_count].Contact(k)-RS[frame_count].Slip(k);
        ...
        for (int k = 0; k < robot.leg_no; k++) {
            if (RS[frame_count].Hard_Contact(k) && RS[frame_count - 1].Hard_Contact(k)) {
                ...
                RS[frame_count].d.block(3 * k, 0, 3, 1) = RS[frame_count].Position + RS[frame_count].Rotation *
                                                                                     robot.Forward_Kinematics_Leg(
                                                                                             ENCODER.block<3, 1>(3 * k,
                                                                                                                 0),
                                                                                             k + 1);
            }
        }
    ...
    if (frame_count > 0) {
        ...
        for (int k = 0; k < robot.leg_no; k++) {
            if (RS[i].Hard_Contact(k) && RS[j].Hard_Contact(k)) {
                prop_primitive_sqrt_info.block<3, 3>(15 + 3 * count, 15 + 3 * count) =
                        Eigen::MatrixXd::Identity(3, 3) / sqrt(contact_cov_array_temp[k]);
                count++;
            }
        }
    ...
    for (int k = 0; k < robot.leg_no; k++) {
        if (RS[j].Hard_Contact(k) == true) {
            ...
            kin_k.meas_primitive_sqrt_info =
                    robot.SQRT_INFO_Covariance_Encoder * robot.Jacobian_Leg(ENC, k + 1).inverse();
    ...
    for (int p = 0; p <= frame_count; p++) {
        for (int k = 0; k < robot.leg_no; k++) {
            RS[p].d_v.block(3 * k, 0, 3, 1) = RS[p].Velocity
                                              + RS[p].Rotation *
                                                robot.Jacobian_Leg(Estimation_Z.block<3, 1>(num_z * p + 6 + 3 * k, 0),
                                                                   k + 1) *
                                                Estimation_Z.block<3, 1>(num_z * p + num_z_imu + num_z_encoder + 3 * k,
                                                                         0)
                                              + RS[p].Rotation * BasicFunctions_Estimator::Hat_so3(
                    Estimation_Z.block<3, 1>(num_z * p, 0) - RS[p].Bias_Gyro) * robot.Forward_Kinematics_Leg(
                    Estimation_Z.block<3, 1>(num_z * p + 6 + 3 * k, 0), k + 1);
        }
        //dv_logger.col(p) = RS[p].d_v;
    ...
    for (int p = 0; p < frame_count; p++) {
        ...
        for (int k = 0; k < robot.leg_no; k++) {
            if (RS[p].Hard_Contact(k) && RS[p + 1].Hard_Contact(k)) {
                fac_info[p].prop_primitive_sqrt_info.block<3, 3>(15 + 3 * count, 15 + 3 * count) =
                        Eigen::MatrixXd::Identity(3, 3) / sqrt(contact_cov_array_temp[k]);
                count++;
            }
        }
...
void Z1_estimator::Optimization_Solve() {

    cost_array.setZero();

    Eigen::MatrixXd Jacobian_Vector;
    Eigen::MatrixXd Jacobian_Vector_redundant;
...
//        if (time_count == 20040) {
//            cout << "bg cov" << endl << Marginalized_H.block(0, 0, 3, 3) << endl;
//            cout << "ba cov" << endl << Marginalized_H.block(3, 3, 3, 3) << endl;
//            cout << "R cov" << endl << Marginalized_H.block(6, 6, 3, 3) << endl;
//            cout << "v cov" << endl << Marginalized_H.block(9, 9, 3, 3) << endl;
//            cout << "p cov" << endl << Marginalized_H.block(12, 12, 3, 3) << endl;
//            cout << "avg cov" << endl << Marginalized_H.norm() << endl;
//        }
...
void Z1_estimator::retract_manifold(int start_frame) {
...
//                }
        ...
        X_s = BasicFunctions_Estimator::Expm_seK_Vec(delta_Zeta_Xi.block(RS[i].para_idx + 6, 0, RS[i].para_size - 6, 1),
                                                     2 + RS[i].contact_leg_num) * X_s;
...
void Z1_estimator::update_dv(int start_frame) {
    ...
    for (int i = start_frame; i <= frame_count; i++) {
        ...
        for (int k = 0; k < robot.leg_no; k++) {
            ...
            RS[i].d_v.block(3 * k, 0, 3, 1) = RS[i].Velocity
                                              + RS[i].Rotation *
                                                robot.Jacobian_Leg(Estimation_Z.block<3, 1>(num_z * i + 6 + 3 * k, 0),
                                                                   k + 1) *
                                                Estimation_Z.block<3, 1>(num_z * i + num_z_imu + num_z_encoder + 3 * k,
                                                                         0)
                                              + RS[i].Rotation * BasicFunctions_Estimator::Hat_so3(
                    Estimation_Z.block<3, 1>(num_z * i, 0) - RS[i].Bias_Gyro) * robot.Forward_Kinematics_Leg(
                    Estimation_Z.block<3, 1>(num_z * i + 6 + 3 * k, 0), k + 1);
...
void Z1_estimator::send_states(ROBOT_STATES &state_) {
    ...
    w_gyro =
            RS[frame_count].Rotation * (Estimation_Z.block(num_z * (frame_count), 0, 3, 1) - RS[frame_count].Bias_Gyro);
...
void Z1_estimator::sliding_window() {
...
void Z1_estimator::SAVE_onestep_Z1(int cnt) {
    if (cnt < SAVEMAXCNT) {
        if (textfile_flag) {
            //Sending True States----------------------------------------------------------------------------
            int j = time_count + 1;
            ...
            for (int i = 0; i < robot.leg_no; i++) {
                ...
                dv_true = v_true
                          + R_true *
                            robot.Jacobian_Leg(Estimation_Z.block<3, 1>(num_z * frame_count + 6 + 3 * i, 0), i + 1) *
                            Estimation_Z.block<3, 1>(num_z * frame_count + num_z_imu + num_z_encoder + 3 * i, 0)
                          +
                          R_true * BasicFunctions_Estimator::Hat_so3(Estimation_Z.block<3, 1>(num_z * frame_count, 0)) *
                          robot.Forward_Kinematics_Leg(Estimation_Z.block<3, 1>(num_z * frame_count + 6 + 3 * i, 0),
                                                       i + 1);

                if (SAVE_Z1[idx_TRUE_Contact + i][cnt]) {

                    if (dv_true.norm() > robot.slip_threshold) {
                        SAVE_Z1[idx_TRUE_Slip + i][cnt] = true;
                    } else {
                        SAVE_Z1[idx_TRUE_Slip + i][cnt] = false;
                    }
...
//            for (int i = 0; i < robot.leg_no; i++) {
//                for (int k = 0; k < 3; k++) {
//                    SAVE_Z1[idx_dv_debugging + 12 * p + 3 * i + k][cnt] = dv_logger(3 * i + k, p);
//                }
//            }
//        }
...
void Z1_estimator::do_SAVE_Z1_all(std::string cov_info) {
    ...
    str = str + estimator_info + "_" + cov_info + "_" + initial_info + "_" + file_info + "x" +
          std::to_string(time_count) + "_" + std::to_string(WINDOW_SIZE) + ".txt";
...
void Z1_estimator::Onestep(Eigen::Matrix<double, 30, 1> &Sensor_i, Eigen::Matrix<bool, 4, 1> &Contact_i,
                           ROBOT_STATES &state_) {
    ...
    if ( (RS[frame_count].Velocity.norm() > 100 || RS[frame_count].Velocity.norm()==Eigen::Infinity ) && dt != 0) {
